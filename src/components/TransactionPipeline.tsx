...
import BraDetailsModal, { BraDetails } from './BraDetailsModal';
import {
...
export default function TransactionPipeline({ lead, onUpdate }: TransactionPipelineProps) {
  ...
  const [showBraModal, setShowBraModal] = useState(false);
  const [braAgentMeta, setBraAgentMeta] = useState<{ name: string; brokerage: string; phone: string; address: string } | null>(null);
  ...
  const handleSendEmail = async (stepId: string) => {
    // BRA requires extra agent details — show modal first
    if (stepId === 'bra') {
      const currentUser = auth.currentUser;
      const agentSnap = await getDoc(doc(db, 'agents', currentUser!.uid));
      const agentData = agentSnap.exists() ? agentSnap.data() : {};
      ...
      setShowBraModal(true);
      return;
    }
    await doSendEmail(stepId, null);
  };

  const handleBraConfirm = async (details: BraDetails) => {
    setShowBraModal(false);
    await doSendEmail('bra', details);
  };

  const doSendEmail = async (stepId: string, braDetails: BraDetails | null) => {
    ...
    try {
      ...
      // BRA: save agent details and prefill PDF BEFORE sending the email so the
      // document is ready the moment the lead clicks the link.
      if (stepId === 'bra' && braDetails) {
        // 1. Persist agent details to Firestore
        await setDoc(doc(db, 'leads', lead.id, 'braDetails', 'agent'), {
          ...braDetails,
          savedAt: new Date().toISOString(),
        });

        // 2. Generate the filled PDF (Gemini Vision call — ~5-10 s)
        await prefillBraWithDetails(braDetails);
      }
      ...
      // Non-BRA steps: prefill in background after email sends
      if (stepId !== 'bra') {
  ...
  const prefillBraWithDetails = async (details: BraDetails) => {
    const currentUser = auth.currentUser;
    if (!currentUser) return;
    const agentSnap = await getDoc(doc(db, 'agents', currentUser.uid));
    const agentData = agentSnap.exists() ? agentSnap.data() : {};

    // Fetch the FINTRAC-prefilled PDF from Firestore to use as the base.
    // The server will draw agent fields on top of the buyer data already there.
    let basePdfBase64: string | undefined;
    try {
      const fintracPrefillSnap = await getDoc(doc(db, 'leads', lead.id, 'prefillCache', 'bra'));
      if (fintracPrefillSnap.exists()) {
        const cached = fintracPrefillSnap.data();
        if (cached.pdf && cached.fieldsFilled > 0) {
          basePdfBase64 = cached.pdf;
          console.log('[BRA] Using FINTRAC-prefilled PDF as base for agent data overlay');
        }
      }
    } catch { /* ignore — server will use blank BRA as fallback */ }

    const propertyAddress = [
      details.propertyStreet,
      details.propertyUnit ? `Unit ${details.propertyUnit}` : '',
      details.propertyCity,
      details.propertyProvince,
      details.propertyPostalCode,
    ].filter(Boolean).join(', ');

    const res = await fetch('/api/prefill-document', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        stepId: 'bra',
        // No cacheKey — server must run fresh. Result is saved to Firestore below.
        ...(basePdfBase64 ? { basePdfBase64 } : {}),
        // Always include leadData — server uses it if basePdfBase64 turns out to be stale/invalid
        leadData: {
          name: (lead as any).fintracData?.fullName || lead.name,
          email: lead.email,
          address: (lead as any).fintracData?.address || (lead as any).currentAddress || '',
          dateOfBirth: (lead as any).fintracData?.dateOfBirth || '',
        },
        agentData: {
          name: agentData.name || currentUser.displayName || '',
          email: agentData.email || currentUser.email || '',
          brokerage: details.brokerageName || agentData.brokerage || '',
          brokeragePhone: details.brokeragePhone,
          brokerageAddress: details.brokerageAddress,
          startDate: details.startDate,
          expiryDate: details.expiryDate,
          commission: details.commission,
          propertyType: details.propertyType,
          propertyAddress,
          geographicLocation: details.geographicLocation,
          agentSignature: details.agentSignatureDataUrl,
          agentInitials: details.agentInitialsDataUrl,
        },
      }),
    });

    if (res.ok) {
      const { pdf, fieldsFilled } = await res.json();
      if (pdf) {
        // PDF is ~6MB as base64 — too large for Firestore (1MB limit).
        // Upload to Firebase Storage and save only the download URL to Firestore.
        const pdfBytes = Uint8Array.from(atob(pdf), c => c.charCodeAt(0));
        const storageRef = ref(storage, `braFilled/${lead.id}/bra.pdf`);
        await uploadBytes(storageRef, pdfBytes, { contentType: 'application/pdf' });
        const downloadUrl = await getDownloadURL(storageRef);
        await setDoc(doc(db, 'leads', lead.id, 'prefillCache', 'bra'), {
          pdfUrl: downloadUrl,
          fieldsFilled,
          cachedAt: new Date().toISOString(),
        });
        console.log(`[BRA] Combined prefill uploaded to Storage (fieldsFilled=${fieldsFilled})`);
      }
    }
  };
  ...
  return (
    <div className="space-y-4">
      {showBraModal && braAgentMeta && (
        <BraDetailsModal
          agentName={braAgentMeta.name}
          agentBrokerage={braAgentMeta.brokerage}
          agentPhone={braAgentMeta.phone}
          agentAddress={braAgentMeta.address}
          onConfirm={handleBraConfirm}
          onCancel={() => setShowBraModal(false)}
        />
      )}
      {/* Progress bar */}
