...
async function prefillBraBackground(params: {
  leadId: string;
  extractedData: Record<string, string>;
  leadEmail: string;
  agentBraUrl: string | null;
  agentName: string;
  agentEmail: string;
  agentBrokerage: string;
  setDoc: (pdf: string, fieldsFilled: number) => Promise<void>;
}) {
  const { leadId, extractedData, leadEmail, agentBraUrl, agentName, agentEmail, agentBrokerage, setDoc } = params;
  try {
    const res = await fetch('/api/prefill-document', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        stepId: agentBraUrl ? null : 'bra',
    ...
    if (pdf && fieldsFilled > 0) {
      await setDoc(pdf, fieldsFilled);
      console.log(`[FINTRAC] BRA prefill cached (fieldsFilled=${fieldsFilled})`);
    }
  } catch (e) {
    console.warn('[FINTRAC] Background BRA prefill failed (non-fatal):', e);
  }
}

function DoneScreen({ leadEmail }: { leadEmail: string }) {
  const [countdown, setCountdown] = useState(5);

  useEffect(() => {
    const interval = setInterval(() => {
      setCountdown(c => {
        if (c <= 1) { clearInterval(interval); window.close(); }
        return c - 1;
      });
    }, 1000);
  ...
  return (
    <FintracLayout>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-5 py-8">
        ...
        <p className="text-xs text-charcoal/40">This window will close in {countdown} second{countdown !== 1 ? 's' : ''}…</p>
      </motion.div>
    </FintracLayout>
  );
}
...
export default function FintracUpload() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Show done immediately — BRA prefill runs in background
      setStage('done');

      // Fire-and-forget: prefill BRA using extracted FINTRAC data, cache to Firestore
      // Lead sees "done" instantly; BRA will be ready by the time agent sends the link
      prefillBraBackground({
        leadId,
        extractedData,
        leadEmail,
        agentBraUrl,
        ...
        agentEmail,
        agentBrokerage,
        setDoc: (pdf: string, fieldsFilled: number) =>
          setDoc(doc(db, 'leads', leadId, 'prefillCache', 'bra'), {
            pdf, fieldsFilled, cachedAt: new Date().toISOString(),
          }),
      });
    } catch (err: any) {
  ...
  // ── Done ─────────────────────────────────────────────────────────────────
  if (stage === 'done') {
    return <DoneScreen leadEmail={leadEmail} />;
  }
