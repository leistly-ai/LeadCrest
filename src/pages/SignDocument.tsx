...
export default function SignDocument() {
  ...
  // Tracks the Firebase Storage URL separately so the sign endpoint can fetch server-side
  // (avoids CORS issues when the browser tries to fetch the PDF from Storage)
  ...
  // Mortgage doc upload state — per-category, never persisted to DB
  const [docCategoryFiles, setDocCategoryFiles] = useState<Record<string, IdFile[]>>({});
  const [docUploadError, setDocUploadError] = useState('');
  const activeDocCategoryRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Only resolve a default PDF URL for steps that are known to have a PDF file.
      // Using step.hasPdf avoids a HEAD request that Vite's SPA fallback would intercept
      // and return 200 for, even when public/documents/<stepId>.pdf doesn't exist.
      if (!resolvedPdfUrl && step?.hasPdf) {
        resolvedPdfUrl = `/documents/${stepId}.pdf`;
      }

      // Read agent BRA modal details — used to enrich prefill and detect stale cache
      let braAgentDetails: Record<string, any> | null = null;
      if (stepId === 'bra') {
        const braDetailsDoc = await getDoc(doc(db, 'leads', leadId, 'braDetails', 'agent'));
        if (braDetailsDoc.exists()) braAgentDetails = braDetailsDoc.data();
      }

      // Check Firestore prefill cache — only use it if it was written AFTER the FINTRAC submission
      // AND after any agent BRA modal details were saved (agent details take precedence).
      if (stepId) {
        const prefillCacheDoc = await getDoc(doc(collection(db, 'leads', leadId, 'prefillCache'), stepId));
        if (prefillCacheDoc.exists()) {
          const cached = prefillCacheDoc.data();
          const fintracSubmittedAt = data.fintracData?.submittedAt;
          const braDetailsSavedAt = braAgentDetails?.savedAt;
          const cacheIsStale =
            (fintracSubmittedAt && cached.cachedAt && cached.cachedAt < fintracSubmittedAt) ||
            (braDetailsSavedAt && cached.cachedAt && cached.cachedAt < braDetailsSavedAt);
          const cachedPdfUrl: string | undefined = cached.pdfUrl; // Storage URL
          const cachedPdfBase64: string | undefined = cached.pdf;  // legacy inline base64
          if ((cachedPdfUrl || cachedPdfBase64) && !cacheIsStale) {
            console.log(`[SignDoc] Firestore cache hit for ${stepId} (fieldsFilled=${cached.fieldsFilled})`);
            if (cachedPdfUrl) {
              // Stored in Firebase Storage — use URL directly for display
              setPdfUrl(cachedPdfUrl);
              // Save Storage URL so the sign endpoint can fetch it server-side (avoids CORS)
              setPdfStorageUrl(cachedPdfUrl);
              // Also attempt a client-side fetch for prefillPdfBase64 (best-effort, may fail due to CORS)
              fetch(cachedPdfUrl)
                ...
                .then(buf => {
                  ...
                  setPrefillPdfBase64(btoa(binary));
                })
                .catch(() => { /* CORS blocked — server will fetch via pdfStorageUrl instead */ });
            } else {
              // Legacy inline base64
              setPrefillPdfBase64(cachedPdfBase64!);
              const bytes = new Uint8Array(atob(cachedPdfBase64!).split('').map(c => c.charCodeAt(0)));
          ...
          if (cacheIsStale) {
            console.log(`[SignDoc] Cache stale (cached=${cached.cachedAt}, fintrac=${fintracSubmittedAt}, braDetails=${braDetailsSavedAt}) — regenerating`);
          }
        }
      }
      ...
      if (resolvedPdfUrl) {
        ...
        // Build agentData — merge BRA modal details on top of basic agent profile
        const resolvedAgentData: Record<string, any> = {
          ...resolvedAgentInfo,
          date: new Date().toLocaleDateString('en-CA', { year: 'numeric', month: 'long', day: 'numeric' }),
        };
        if (braAgentDetails) {
          resolvedAgentData.brokerage      = braAgentDetails.brokerageName || resolvedAgentInfo.brokerage;
          resolvedAgentData.brokeragePhone = braAgentDetails.brokeragePhone || '';
          resolvedAgentData.brokerageAddress = braAgentDetails.brokerageAddress || '';
          resolvedAgentData.startDate      = braAgentDetails.startDate || '';
          resolvedAgentData.expiryDate     = braAgentDetails.expiryDate || '';
          resolvedAgentData.commission     = braAgentDetails.commission || '';
          resolvedAgentData.propertyType   = braAgentDetails.propertyType || '';
          resolvedAgentData.propertyAddress = [
            braAgentDetails.propertyStreet,
            braAgentDetails.propertyUnit ? `Unit ${braAgentDetails.propertyUnit}` : '',
            braAgentDetails.propertyCity,
            braAgentDetails.propertyProvince,
            braAgentDetails.propertyPostalCode,
          ].filter(Boolean).join(', ');
          resolvedAgentData.geographicLocation = braAgentDetails.geographicLocation || '';
          resolvedAgentData.agentSignature  = braAgentDetails.agentSignatureDataUrl || '';
          resolvedAgentData.agentInitials   = braAgentDetails.agentInitialsDataUrl || '';
        }
        ...
        try {
          const prefillRes = await fetch('/api/prefill-document', {
            ...
            body: JSON.stringify({
              ...
              // Skip server cache for BRA — we always want fresh data from braDetails
              ...(stepId !== 'bra' && { cacheKey: `${leadId}:${stepId}` }),
              leadData: {
                // Prefer verified legal name/address from FINTRAC ID over self-reported data
                name: data.fintracData?.fullName || name,
                email,
                address: data.fintracData?.address || data.currentAddress,
                dateOfBirth: data.fintracData?.dateOfBirth || '',
                phone: data.phone,
          ...
          if (prefillRes.ok) {
            ...
            if (fieldsFilled > 0) {
              ...
              setPdfUrl(URL.createObjectURL(new Blob([bytes], { type: 'application/pdf' })));
              // Persist to Storage so next load is instant
              if (stepId === 'bra') {
                (async () => {
                  try {
                    const bytes = Uint8Array.from(atob(pdf), c => c.charCodeAt(0));
                    const sRef = ref(storage, `braFilled/${leadId}/bra.pdf`);
                    await uploadBytes(sRef, bytes, { contentType: 'application/pdf' });
                    const url = await getDownloadURL(sRef);
                    await setDoc(doc(db, 'leads', leadId!, 'prefillCache', 'bra'), {
                      pdfUrl: url, fieldsFilled, cachedAt: new Date().toISOString(),
                    });
                  } catch { /* non-fatal */ }
                })();
              }
  ...
  const startDrawInitials = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    const canvas = initialsCanvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d')!;
    ...
    setInitialsEmpty(false);
  };

  const moveDrawInitials = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    if (initialsEmpty && !initialsCanvasRef.current) return;
    const canvas = initialsCanvasRef.current;
    if (!canvas) return;
    const ctx = canvas.getContext('2d')!;
    const r = canvas.getBoundingClientRect();
    const src = 'touches' in e ? e.touches[0] : e;
    ctx.lineTo(src.clientX - r.left, src.clientY - r.top); ctx.stroke();
    setInitialsDataUrl(canvas.toDataURL('image/png'));
  };

  // ── ID upload helpers (FINTRAC) ───────────────────────────────────────────
  ...
  // ── Mortgage doc upload helpers ───────────────────────────────────────────
  const openDocUpload = (catId: string) => {
    setDocUploadError('');
    activeDocCategoryRef.current = catId;
    docFileInputRef.current?.click();
  };

  const handleDocFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    const catId = activeDocCategoryRef.current;
    if (!catId) return;
    setDocUploadError('');
    const files = Array.from(e.target.files || []);
    if (!files.length) return;

    const cat = step?.docUploadCategories?.find(c => c.id === catId);
    const maxFiles = cat?.maxFiles ?? 5;
    const existing = docCategoryFiles[catId] || [];
    const remaining = maxFiles - existing.length;
    const toProcess = files.slice(0, remaining);

    toProcess.forEach(file => {
      if (file.size > MAX_FILE_MB * 1024 * 1024) {
        setDocUploadError(`"${file.name}" exceeds ${MAX_FILE_MB} MB. Please use a smaller file.`);
        return;
      }
      const reader = new FileReader();
      reader.onload = ev => {
        const dataUrl = ev.target?.result as string;
        setDocCategoryFiles(prev => ({
          ...prev,
          [catId]: [...(prev[catId] || []), {
            name: file.name,
            dataUrl,
  ...
  const removeDocFile = (catId: string, idx: number) => {
    setDocCategoryFiles(prev => ({
      ...prev,
  ...
  const nextAfterSummary = () => {
    if (step?.requiresIdUpload || step?.requiresDocUpload) return setFlowStep('upload');
    if (step?.requiresSignature) return setFlowStep('sign');
    setFlowStep('preview');
  };

  const nextAfterUpload = () => {
    // Mortgage doc upload: no signature needed — go straight to preview
    if (step?.requiresDocUpload) return setFlowStep('preview');
    // FINTRAC: has a consent acknowledgement step on the sign screen
    setFlowStep('sign');
  };

  const handleGoToPreview = () => {
    const braRequiresInitials = stepId === 'bra';
    if (step?.requiresSignature && signatureEmpty) return;
    if (braRequiresInitials && initialsEmpty) return;
    if (step?.requiresSignature) {
      setSignatureDataUrl(canvasRef.current!.toDataURL('image/png'));
    }
    if (braRequiresInitials && initialsCanvasRef.current) {
      setInitialsDataUrl(initialsCanvasRef.current.toDataURL('image/png'));
    }
    setFlowStep('preview');
  };
    ...
    try {
      ...
      await fetch('/api/sign-document', {
        ...
        body: JSON.stringify({
          ...
          signedAt,
          // ID files + mortgage docs sent inline — server attaches to email, never writes to disk/DB
          idAttachments: [
            ...idFiles.map(f => ({
              filename: f.name,
              content: f.dataUrl.split(',')[1],
              mimeType: f.mimeType,
            })),
            ...Object.entries(docCategoryFiles).flatMap(([catId, files]) => {
              const cat = step?.docUploadCategories?.find(c => c.id === catId);
              return files.map(f => ({
                filename: `[${cat?.label ?? catId}] ${f.name}`,
                content: f.dataUrl.split(',')[1],
                mimeType: f.mimeType,
              }));
            }),
          ],
          // Pre-filled PDF (base64) — client sends this if it could load it
          prefillPdf: prefillPdfBase64 || null,
          // Firebase Storage URL — server fetches PDF directly if prefillPdf is empty (avoids CORS)
          pdfStorageUrl: pdfStorageUrl || null,
          // Buyer initials (BRA only)
          initialsDataUrl: initialsDataUrl || null,
        }),
      });
        ...
        >
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-midnight">
              {step.requiresIdUpload ? 'Verification Complete!' : step.requiresDocUpload ? 'Documents Submitted!' : 'Document Signed!'}
            </h2>
            <p className="text-charcoal/60 text-sm">
              {step.requiresIdUpload
                ...
                : step.requiresDocUpload
                ? `Your mortgage documents have been securely forwarded to your agent. A confirmation will be sent to `
        ...
        <motion.div key="summary" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
          ...
          <div className="border-l-4 border-honey pl-4 py-1">
            <p className="text-xs font-bold text-charcoal/50 uppercase tracking-widest mb-1">
              {step.requiresSignature ? 'By signing, you confirm:' : (step.requiresIdUpload || step.requiresDocUpload) ? 'By submitting, you confirm:' : 'By acknowledging, you confirm:'}
            </p>
          ...
          {step.requiresDocUpload && (
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <div>
                <p className="text-sm font-bold text-amber-800">Documents Required for This Step</p>
                <p className="text-xs text-amber-700 mt-1">You will be asked to upload your financial documents on the next screen. Files are forwarded directly to your agent via encrypted email and are <strong>not stored</strong> in this application.</p>
              </div>
            </div>
          )}
          ...
          >
            {step.requiresIdUpload ? 'Continue to ID Upload' : step.requiresDocUpload ? 'Continue to Document Upload' : step.requiresSignature ? 'Continue to Sign' : 'Continue to Review'}
  ...
  // ── Step 2a: ID Upload (FINTRAC) ─────────────────────────────────────────
  if (flowStep === 'upload' && step.requiresIdUpload) {
    return (
      <SignLayout flowStep={flowStep} step={step}>
  ...
  // ── Step 2b: Mortgage Document Upload ─────────────────────────────────────
  if (flowStep === 'upload' && step.requiresDocUpload) {
    const categories = step.docUploadCategories || [];
    const allRequiredUploaded = categories
      .filter(c => c.required)
      .every(c => (docCategoryFiles[c.id] || []).length > 0);
    const totalFiles = Object.values(docCategoryFiles).reduce((sum, files) => sum + files.length, 0);

    return (
      <SignLayout flowStep={flowStep} step={step}>
        <motion.div key="doc-upload" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
          <div>
            <button onClick={() => setFlowStep('summary')} className="flex items-center gap-1 text-xs text-charcoal/40 hover:text-midnight transition-colors font-bold mb-4">
              <ArrowLeft className="w-3 h-3" /> Back
            </button>
            <h2 className="text-xl font-black text-midnight">Upload Your Documents</h2>
            <p className="text-sm text-charcoal/50 mt-1">Please upload each document below. All files are forwarded directly to your agent.</p>
          </div>
          ...
          <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl p-4">
            ...
            <div className="text-xs text-blue-800 space-y-1">
              <p className="font-bold">Your privacy is protected</p>
              <p>Documents are sent directly to your agent via encrypted email. They are <strong>not stored</strong> in LeadCrest's database or servers at any point.</p>
            </div>
          </div>

          {/* Hidden file input shared across all categories */}
          <input
            ref={docFileInputRef}
            type="file"
            accept="image/jpeg,image/png,image/heic,application/pdf"
          ...
          {/* Per-category upload slots */}
          <div className="space-y-5">
            {categories.map(cat => {
              const catFiles = docCategoryFiles[cat.id] || [];
              const atMax = catFiles.length >= (cat.maxFiles ?? 5);
              const hasFile = catFiles.length > 0;
              return (
                <div key={cat.id} className="border border-zinc-200 rounded-2xl overflow-hidden">
                  {/* Category header */}
                  <div className={`px-4 py-3 flex items-center justify-between ${hasFile ? 'bg-sage/10 border-b border-sage/20' : 'bg-zinc-50 border-b border-zinc-100'}`}>
                    <div>
                      <p className="text-sm font-black text-midnight">{cat.label}</p>
                      {cat.hint && <p className="text-[11px] text-charcoal/50 mt-0.5">{cat.hint}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      {hasFile
                        ? <span className="text-[10px] font-bold text-sage bg-sage/10 border border-sage/30 px-2 py-0.5 rounded-full">{catFiles.length} file{catFiles.length > 1 ? 's' : ''}</span>
                        : cat.required && <span className="text-[10px] font-bold text-red-500 uppercase tracking-wide">Required</span>
                      }
                    </div>
                  </div>
                    ...
                    <div className="divide-y divide-zinc-100">
                      {catFiles.map((f, i) => (
                        <div key={i} className="flex items-center gap-3 px-4 py-3 bg-white">
                          <FileImage className="w-4 h-4 text-honey shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold text-midnight truncate">{f.name}</p>
                            <p className="text-[10px] text-charcoal/40">{f.sizeKb} KB</p>
                          </div>
                          <button onClick={() => removeDocFile(cat.id, i)} className="text-charcoal/30 hover:text-red-500 transition-colors p-1">
                  ...
                  {!atMax && (
                    <button
                      onClick={() => openDocUpload(cat.id)}
                      className="w-full px-4 py-3 flex items-center gap-2 text-sm text-charcoal/50 hover:text-honey hover:bg-zinc-50 transition-colors border-t border-dashed border-zinc-200"
                    >
                      <Upload className="w-4 h-4" />
                      <span>{hasFile ? 'Add another file' : 'Upload file'}</span>
                      <span className="text-[10px] text-charcoal/30 ml-auto">JPEG, PNG, HEIC, PDF · max {MAX_FILE_MB} MB</span>
                    </button>
                  )}
                </div>
              );
          ...
          <AnimatePresence>
            {docUploadError && (
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-sm text-red-500 bg-red-50 rounded-xl p-3 flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 shrink-0" /> {docUploadError}
              </motion.p>
            )}
          </AnimatePresence>
          ...
          <button
            ...
            className="w-full py-4 bg-honey text-white font-black rounded-2xl hover:bg-honey/90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg text-sm"
          >
            Review & Submit ({totalFiles} file{totalFiles !== 1 ? 's' : ''}) <ArrowRight className="w-4 h-4" />
          </button>
          <p className="text-[10px] text-charcoal/30 text-center">All required documents must be uploaded before you can proceed.</p>
        </motion.div>
      </SignLayout>
    );
  }
    ...
    return (
      <SignLayout flowStep={flowStep} step={step}>
        <motion.div key="sign" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
          ...
          {/* Initials — required for BRA only */}
          {stepId === 'bra' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-midnight flex items-center gap-2">
                  <PenLine className="w-4 h-4 text-honey" /> Your Initials
                </p>
                <button onClick={clearInitials} className="flex items-center gap-1 text-xs text-charcoal/40 hover:text-red-500 transition-colors font-bold">
                  <RotateCcw className="w-3 h-3" /> Clear
                </button>
              </div>
              <div className="relative border-2 border-dashed border-zinc-300 rounded-2xl overflow-hidden bg-white touch-none" style={{ height: 90 }}>
                <canvas
                  ...
                  onMouseMove={moveDrawInitials}
                  onMouseUp={() => {}}
                  onMouseLeave={() => {}}
                  onTouchStart={startDrawInitials}
                  onTouchMove={moveDrawInitials}
                  onTouchEnd={() => {}}
                />
                {initialsEmpty && (
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <p className="text-zinc-300 text-sm font-medium">Initials here</p>
                  </div>
                )}
              </div>
              <p className="text-[10px] text-charcoal/30 text-center">× Initials of {leadName}</p>
            </div>
          )}

          <button
            onClick={handleGoToPreview}
            disabled={signatureEmpty || (stepId === 'bra' && initialsEmpty)}
            className="w-full py-4 bg-honey text-white font-black rounded-2xl hover:bg-honey/90 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg text-sm"
          >
  ...
  const backToSign: FlowStep = step.requiresSignature ? 'sign' : (step.requiresIdUpload || step.requiresDocUpload) ? 'upload' : 'summary';
