export interface DocUploadCategory {
  id: string;
  label: string;
  hint?: string;         // e.g. "2–3 most recent"
  required: boolean;
...
export interface StepDefinition {
  ...
  // Requires a drawn e-signature (vs. a simple acknowledgement click)
  requiresSignature: boolean;
  // Lead must upload government-issued photo ID (FINTRAC step only)
  requiresIdUpload: boolean;
  // Lead must upload categorised financial documents (mortgage-docs step)
  requiresDocUpload?: boolean;
  docUploadCategories?: DocUploadCategory[];
  // A PDF file exists at public/documents/<id>.pdf and should be prefilled/attached
  // Steps without this flag are acknowledgement-only (no PDF document to display)
  hasPdf?: boolean;
}
...
export const PIPELINE_STEPS: StepDefinition[] = [
  ...
  },
  {
    ...
    requiresSignature: false,
    requiresIdUpload: false,
    requiresDocUpload: true,
    docUploadCategories: [
      { id: 'employment-letter', label: 'Employment Letter', hint: 'On company letterhead', required: true, maxFiles: 2 },
      { id: 'pay-stubs', label: 'Pay Stubs', hint: '2–3 most recent', required: true, maxFiles: 3 },
      { id: 'noa', label: 'Notice of Assessment (NOA)', hint: 'Last 2 years — upload each year separately', required: true, maxFiles: 2 },
      { id: 'down-payment', label: 'Proof of Down Payment', hint: '90-day bank statements for all contributing accounts', required: true, maxFiles: 5 },
    ],
